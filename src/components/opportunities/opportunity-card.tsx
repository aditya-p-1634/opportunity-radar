...
import { Bookmark, BookmarkCheck, ExternalLink, MapPin, Clock, BadgeCheck, Banknote } from "lucide-react";
  ...
  return (
    <article className="group relative flex flex-col rounded-2xl border border-zinc-200/80 bg-white p-5 shadow-sm transition hover:border-indigo-300/60 hover:shadow-md dark:border-zinc-800 dark:bg-zinc-950 dark:hover:border-indigo-500/40">
      <div className="mb-3 flex items-start justify-between gap-3">
        ...
        {opportunity.matchScore != null && (
          <div className="flex flex-col items-end">
            ...
            <span className="text-[10px] uppercase tracking-wide text-zinc-400">match</span>
          </div>
        )}
      </div>
      ...
      {opportunity.researchArea && (
        <p className="mb-3 text-sm text-zinc-500 dark:text-zinc-400">{opportunity.researchArea}</p>
      )}

      {opportunity.matchReasons && opportunity.matchReasons.length > 0 && (
        <div className="mb-3 flex flex-wrap gap-1.5">
          {opportunity.matchReasons.slice(0, 4).map((r) => (
      ...
      <div className="mt-auto grid grid-cols-2 gap-2 text-xs text-zinc-500 dark:text-zinc-400">
        <span className="flex items-center gap-1.5">
          ...
          {truncate(opportunity.location || opportunity.country || modeLabel(opportunity.mode || "ONSITE"), 28)}
        </span>
        <span className="flex items-center gap-1.5">
...
export function InstitutionAvatar({
  ...
  // Always use initials avatar — SVG logos are generated as data URIs via gradient
  const colors = [
